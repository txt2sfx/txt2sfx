...
> **Отменено 2026-08-09 после проверки в браузере.** Jog неотменяем: вернуться к исходным
> числам можно только новым перетаскиванием на глаз. Мастер теперь якорится на рецепте
> (`MasterAnchor`, позиция абсолютна, центр = рецепт как он открылся), якорь сбрасывается
> при любой правке не от мастера, а зафиксировать сдвиг отдельным рецептом можно кнопкой
> «⎘ в новый». Разбор — п. 27 в `.plans/txt2sfx-v0.md`; ниже — исходное решение фазы.
