...
export const ru: Partial<Readonly<Record<Key, string>>> = {
  ...
  'search.keyLabel': 'ключ freesound.org',
  'search.keyPlaceholder': 'API-ключ',
  'search.keyAria': 'API-ключ freesound.org',
  'search.remember': 'запомнить',
  'search.rememberTitle': 'шифруется в этом браузере ключом, который нельзя выгрузить',
  'search.forget': 'забыть',
  'search.getKey': 'получить ключ ↗',
  ...
  'search.needsKey': 'Вставьте ключ freesound.org выше. Запросы уходят из этой вкладки в библиотеку и больше никуда.',
  ...
  'search.downloadTitle': 'скачать превью (mp3)',
  ...
  'search.errKey': 'freesound.org не принял ключ. Проверьте его или получите новый по ссылке выше.',
  'search.errThrottled': 'freesound.org ограничивает этот ключ. Подождите минуту и повторите.',
  ...
  'search.caption':
    'Это цель, а не то, что вы поставите в игру: кнопка отдаёт превью (mp3), а оригинал лежит на странице звука под лицензией с его бейджа.',
