...
export const ru: Partial<Readonly<Record<Key, string>>> = {
  ...
  'loop.placeholderNew': 'опишите саундтрек или соберите его из чипов ниже',
  'loop.placeholderMore': 'добавьте то, чего нет в чипах',
  ...
  /* Про английские слова сказано честно: таблица ключевых слов читает только их, и сами
     чипы поэтому английские — то, что они дописывают, и есть то, что уходит в модель. */
  'loop.hintNew': 'чипы дописывают слова в запрос; запрос решает темп, тональность, регистр и дорожки',
  'loop.hintAdd': 'готовые дорожки не меняются — приходят одна-две новые, чтобы их послушать',
  'loop.hintNewModel': '{model} пишет партитуру по запросу — темп, тональность, партии и каждую ноту',
  ...
  'loop.group.mood': 'НАСТРОЕНИЕ',
  'loop.group.motion': 'ДВИЖЕНИЕ',
  'loop.group.texture': 'ФАКТУРА',
  'loop.group.kit': 'ПАРТИИ',
  'loop.group.place': 'МЕСТО',
  'loop.dropTag': 'убрать «{tag}» из запроса',
  'loop.phase.writing': 'пишу партитуру',
  'loop.phase.checking': 'проверяю её по правилам',
  'loop.phase.voicing': 'собираю инструменты',
  'loop.phase.mixing': 'свожу петлю',
  'loop.phase.trip': 'заход {attempt} из {trips}',
  'loop.phase.repairing': 'прошлая партитура нарушила {rule} — прошу исправить',
  'loop.phase.chars': 'ответ: {chars} символов',
  'loop.meta': '{bars} тактов · петля {length} · дорожек: {lanes}',
  'loop.play': 'Играть петлю',
  'loop.stop': 'Стоп',
  'loop.seamless': '∞ бесшовная петля',
  'loop.bankOff': 'GM-банк',
  'loop.bankOn': 'GM-банк · вкл',
  ...
  'loop.noSpare': 'Все дорожки, которые можно предложить этому треку, уже здесь. Соберите новый.',
  ...
  'loop.msg.epiano': 'прогреваю электропиано…',
  'loop.msg.organ': 'вытягиваю регистры органа…',
  'loop.msg.stabs': 'нарезаю стэбы…',
  'loop.msg.guitar': 'включаю гитару в усилитель…',
  'loop.msg.riff': 'глушу рифф ладонью…',
  'loop.msg.brass': 'выстраиваю медную секцию…',
